...
load(file.path(data_dir, 'bcell_down_sampled_train_MMD_VAE_sce.RData'))   # sce_norm
load(file.path(data_dir, 'bcell_down_sampled_test_MMD_VAE_sce.RData'))    # sce_norm_test
...
clinicaldata$y <- factor(if_else(clinicaldata$condition == "COVID", 1, 0))

clinicaldata_test <- colData(sce_norm_test) %>% as.data.frame() %>%
  dplyr::select(sample_id, condition) %>% distinct()
clinicaldata_test$y <- factor(if_else(clinicaldata_test$condition == "COVID", 1, 0))
...
# --- Summary ---
cat(sprintf("\n--- COVID-19 Bootstrap Results (n=%d) ---\n", nBootstrap))
cat(sprintf("Point AUC: %.4f\n", point_auc))
...
write.csv(results, file.path(revision_root, 'results/uncertainty/covid_bootstrap_ci.csv'),
          row.names = FALSE)

boot_df <- data.frame(bootstrap_id = seq_along(boot_aucs), auc = boot_aucs)
write.csv(boot_df, file.path(revision_root, 'results/uncertainty/covid_bootstrap_distribution.csv'),
          row.names = FALSE)
