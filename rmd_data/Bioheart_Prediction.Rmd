...
sce_norm$clusters_norm <- recode_factor(sce_norm$clusters_norm,
                                 "cluster_1" = "Monocytes", # monocytes
                                 "cluster_2" = "Tregs", # Tregs
                                 "cluster_3" = "CM CD4+ T cells", # central memory CD4+ T cells
                                 "cluster_4" = "EMA CD8+ T cells", # effector memory/activated CD8+ T cells
                                 "cluster_5" = "Monocytes/DCs", # mix of monocytes and dendritic cells
                                 "cluster_6" = "CD4+ T cells/B cells/NK cells", # mix of naïve CD4+ T cells, B cells and NK cells
                                 "cluster_7" = "B cells", # B cells
                                 "cluster_8" = "NK cells", # NK cells
                                 "cluster_9" = "CM CD8+ T cells", # central memory CD8+ T cells
                                 "cluster_10" = "Naive CD8+ T cells", # naïve CD8+ T cells
                                 "cluster_11" = "TEMR CD8+ T cells" # terminal effector memory revertant CD8+ T cells
                                 )
```


```{r, eval=FALSE}
p.raw <- assay(sce_norm, 'raw') %>%
  t() %>%
  as.data.frame() %>%
  dplyr::mutate(Sample = as.factor(sce_norm$sample_id)) %>%
  ggplot(aes(x = CD8a)) +
  geom_density(aes(colour = Sample)) +
  theme(legend.position = "none")

p.norm <- assay(sce_norm, 'norm') %>%
  t() %>%
  as.data.frame() %>%
  dplyr::mutate(Sample = as.factor(sce_norm$sample_id)) %>%
  ggplot(aes(x = CD8a)) +
...
cowplot::plot_grid(p.raw, p.norm)
# plot(sce_norm@assays@data$norm["CD3", ], 
#      sce_norm@assays@data$raw["CD3", ])
```
...
pdf(file = "../Plots/Clusters_expression_plots.pdf",   # The directory you want to save the file in
...
train_x <- reducedDim(sce_norm, type = "VAE") %>%
  mutate(cellTypes = as.factor(sce_norm$clusters_norm))
  # mutate(sample_id = sce_norm$sample_id) %>%
...
sce_3_norm$clusters_norm <- as.factor(df_3_clusters)
```
...
p.tree <- visualiseModelTree(fit = fit$fit, tree = tree, type = "MG",
                   training_data = X_train, title = "Annotated Clustering Feature Tree") %>%
  as.ggplot()

# ggsave(file="../Plots/fs_tree.svg", plot=p, width=18, height=16, dpi = 1200)
```
```{r,fig.width=28, fig.height=16, eval=FALSE}
...
## Predict on the training data
```{r}
train_auc <- plotAUC(fit = fit, x_test = X_train, y_test = condition, title = "Study 4 - Proportions AUC =")
train_auc$plot
```
## Predict on test data
```{r}
test_auc <- plotAUC(fit = fit, x_test = X_test_study_3, y_test = condition_3, title = "Study 3 - Proportions AUC =")
test_auc$plot

write.csv(test_auc$preds,
          '../auc_data/fs_glasso_auc_prop_study_3.csv')

```
## Look at tree plot
```{r, fig.height=10, fig.width=16}
visualiseModelTree(fit = fit, tree = tree, type = "MG", 
...
## Predict on the training data
```{r}
train_auc <- plotAUC(fit = fit, x_test = X_train, y_test = condition, title = "Study 4 - Means AUC =")
train_auc$plot
```

## Predict on test data
```{r}
test_auc <- plotAUC(fit = fit, x_test = X_test_study_3, y_test = condition_3, title = "Study 3 - Means AUC =")
test_auc$plot

write.csv(test_auc$preds,
          '../auc_data/fs_glasso_auc_means_study_3.csv')

```

```{r, fig.height=10, fig.width=16}
plotHeatmap(fit, order = order, type = "MG")
```
