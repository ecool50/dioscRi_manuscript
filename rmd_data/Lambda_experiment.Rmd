...
```{r}
nclust = 11
lambda_seq <- seq(0.1, 1, 0.1)
for(i in 1:length(lambda_seq)){
  # fit the model with number of representative samples = i
  lambda <- lambda_seq[i]
  message(paste0("Now fitting for Lambda: ", lambda))
  res <- runNormModel(train = df_scaled, test = df_3_scaled, lambda = lambda,
...
# Combine results
```{r}
res_df <- data.frame(Lambda = lambda_seq,
...
write.csv(res_df,
          '../auc_data/fs_glasso_auc_all_study_lambda.csv')
```
...
auc_plot <- ggplot(df, aes(x = Lambda, y = AUC)) +
...
total_loss_plot <- ggplot(df, aes(x = Lambda)) +
...
```{r}
pdf(file = "../Plots/Manuscript_Plots/Supp_fig2.pdf",   # The directory you want to save the file in
