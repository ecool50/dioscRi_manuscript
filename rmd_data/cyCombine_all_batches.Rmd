...
```{r}
# cellTypes <- c(unique(df_4$CellType), 'T Cells', 'B Cells', 'pDCs')
df <- rbind(df_4, df_3) %>%
  dplyr::filter(CellType %in% final_celltypes)

# update study information
df$batch <- df$batch_old
```
...
```{r, eval=T}
computeConcordance(sce$clusters_cyCombine, sce$CellType)
```
```{r, eval=T}
computeConcordance(sce$clusters_cyCombine, sce$batch)
```
...
# Save SCE object
```{r, eval=T}
save(sce, file = "../sce_dat/bioheart_cyCombine_batch_corrected.RData")
```