---
title: "Deep Learning singe batch"
...
for(batch in batches){
  ...
  train_batch <- df[df$CyTOF.Batch != batch, ]
  test_batch <- df[df$CyTOF.Batch == batch, ]
  ...
  res_ind <- ComputeReferenceSample(train_batch, useMarkers)
  train_dat <- train_batch[which(train_batch$sample_id == res_ind$refSampleInd), useMarkers]
  ...
  train_batch_norm <- predict(models$autoencoder_model, as.matrix(train_batch[, useMarkers])) %>%
  as.data.frame()
  colnames(train_batch_norm) <- useMarkers
  ...
  sce_train_batch <- SingleCellExperiment(assays = list(norm = t(train_batch_norm[, useMarkers])
  ),
  colData = train_batch %>% dplyr::select(-useMarkers))
  
  message("Now training cell type prediction model\n")
  train_batch_norm$clusters_mg <- factor(sce_train_batch$mg_cell_type_distinct)
  train_batch_norm$sample_id <- sce_train_batch$sample_id
  train_batch_norm$batch <- sce_train_batch$CyTOF.Batch
  
  # subset the normalised training batch data to 1000 cells from each population
  # Manually gated
  train_batch_norm_sub_mg <- train_batch_norm %>%
  group_by(clusters_mg) %>%
  slice_sample(n = 1000)
  
  # training MG model
  nnet_model_mg <- fitCelltypeModel(train_x = train_batch_norm_sub_mg[, useMarkers],
                                  train_y = train_batch_norm_sub_mg$clusters_mg)
  ...
  stab_samps <- c(stab_samps, res_ind)
...
write.csv(dat_final_mg, '../concordance_data/deep_learning_single_batches.csv', row.names = F)
```
