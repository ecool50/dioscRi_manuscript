...
test_x <- assay(sce_3_norm, type = "norm") %>%
  t() %>%
  as.data.frame()
```
...
#           '../auc_data/mg_glasso_auc_all_study_3.csv')
...
p.tree.mg <- visualiseModelTree(fit = fit$fit, tree = tree, type = "MG", training_data = X_train,
                   nodes_to_remove =  c("Root", "Myeloid", "CD3", "CD4"), title = "Manual Gating Feature Tree") %>%
  as.ggplot()
# dev.off()
...
stats_mean$stats <- stats_mean$stats %>%
  filter(p.adj < 0.05)
...
## Predict on test data
```{r}
test_auc <- plotAUC(fit = fit, x_test = X_test_study_3, y_test = condition_3, title = "Study 3 MG - Proportions AUC =")
test_auc$plot

write.csv(test_auc$preds,
          '../auc_data/mg_glasso_auc_prop_study_3.csv')
...
## Predict on the training data
```{r}
train_auc <- plotAUC(fit = fit, x_test = X_train, y_test = condition, title = "Study 4 MG - Means AUC =")
train_auc$plot
```
## Predict on test data
```{r}
test_auc <- plotAUC(fit = fit, x_test = X_test_study_3, y_test = condition_3, title = "Study 3 MG - Means AUC =")
test_auc$plot

write.csv(test_auc$preds,
          '../auc_data/mg_glasso_auc_means_study_3.csv')
...
order <- gsub("_", " ", order)
plotHeatmap(fit, type = "MG", order = order)
```
