...
```{r}
computeConcordance(sce$clusters_imubac, sce$CellType)
```

```{r, eval=FALSE}
computeConcordance(sce$clusters_imubac, sce$batch)
```
...
# Save SCE object
```{r, eval=T}
save(sce, file = "../sce_dat/bioheart_iMUBAC_batch_corrected.RData")
```