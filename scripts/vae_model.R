compute_kernel <- function(x, y) {
  ...
  # Reshaping and tiling the inputs for kernel computation
...
train_vae_model <- function(train_dat, useMarkers, epochs = 80, l1_reg = 5e-04,
                            latent_dim = NULL, lambda = 0.1, val_dat,
  ...
  x <- encoder_inputs %>%
    layer_dense(intermediate_dim, activation = "relu",
                kernel_regularizer = regularizer_l1(l1_reg)) %>%
    layer_dense(intermediate_dim_2, activation = "relu",
                kernel_regularizer = regularizer_l1(l1_reg)) %>%
    layer_dense(latent_dim, activation = "gelu")
  
  encoder <- keras_model(encoder_inputs, x, name = "encoder")
  
  # Define the decoder
  decoder_inputs <- layer_input(shape = latent_dim)
  ...
  decoder <- keras_model(decoder_inputs, decoder_outputs, name = "decoder")
  ...
  model_vae <- new_model_class(
    ...
    train_step = function(data) {
      ...
      with(tf$GradientTape() %as% tape, {
        ...
        reconstruction_loss <- loss_binary_crossentropy(x, reconstruction) %>%
          sum(axis = c(1)) %>%
          mean()
        
        true_samples <- tf$random$normal(shape = tf$shape(z_mean))
        mmd_loss <- compute_mmd(true_samples, z_mean)
        total_loss <- reconstruction_loss + lambda * mmd_loss
      })
