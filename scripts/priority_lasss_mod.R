...
prioritylassoMod <- function(X,
  ...
  if (!block1.penalization) {
    ...
    if(cvoffset) {
      ...
      cvdiv <- prioritylasso:::makeCVdivision(n = nrow(X), K = cvoffsetnfolds, nrep = 1)[[1]]
    ...
    result_offsets <- prioritylasso:::calculate_offsets(current_missings = current_missings,
  ...
  for (i in start_block:length(blocks)) {
    ...
    # try and make it adaptive
    message("Trying adaptive lasso")
    set.seed(999)
    cv.ridge <- cv.glmnet(X[current_observations, actual_block], 
                          ...
                          standardize = standardize,parallel = TRUE,
                          ...)
    ...
    lassoerg[[i]] <- cv.glmnet(X[current_observations, actual_block],
                               ...
                               nfolds = nfolds,
                               foldid = foldid[current_observations],
                               standardize = standardize, parallel = T,
    ...
    if(cvoffset) {
      ...
      for(count in seq(along=cvdiv)) {
        ...
        message("Trying adaptive lasso")
        set.seed(999)
        cv.ridge <- cv.glmnet(X[cvdiv[[count]] == 1,actual_block, drop = FALSE], 
                              Y[cvdiv[[count]] == 1],
                              alpha=0, 
                              relax = F,
