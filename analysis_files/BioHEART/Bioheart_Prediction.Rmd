...
sce_norm$clusters_11 <- factor(
  sce_norm$clusters_11,
  levels = c("cluster_5","cluster_7","cluster_3","cluster_6","cluster_9",
             "cluster_10","cluster_8","cluster_11","cluster_2","cluster_1","cluster_4")
)

p <- ggplotify::as.ggplot(scater::plotGroupedHeatmap(sce_norm, 
                           features = useMarkers[order(useMarkers)], 
                           group = cluster_col, 
                           block = cluster_col,
                           exprs_values = "norm",
                           center = TRUE, 
  ...
  # theme_bw() +
  # theme_minimal() +
  ggtitle(label = "Heatmap of unsupervised clustering clusters expression") +
    theme(
          plot.title = element_text(size = 20, hjust = 0.5, face = "bold")
    )

pdf(file = "../../Plots/Supp_Figure_2_heatmap.pdf",   # The directory you want to save the file in
