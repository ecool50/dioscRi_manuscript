...
nclust = 10
sce_imubac$clusters_imubac <- NULL
sce_imubac <- runFuseSOM(sce_imubac, numClusters = nclust, assay = 'normexprs',
                       verbose = FALSE, clusterCol = "clusters_imubac")
```

# Compute concordance on full data
```{r}
computeConcordance(sce_imubac$clusters_imubac, sce_imubac$CellType)
```

```{r, eval=FALSE}
computeConcordance(sce_imubac$clusters_imubac, sce_imubac$batch)
```
...
```{r}
set.seed(1994)
df_norm <- assay(sce_imubac, 'normexprs') %>%
  t() %>%
  as.data.frame()
df_norm$CellType <- factor(sce_imubac$CellType)
df_norm$sample_id <- sce_imubac$sample_id
df_norm$batch <- sce_imubac$batch
```
...
ggplot(data, aes(x = Truth, y = Proportion, fill = Predicted)) +
  ...
  labs(title = "IMUBAC - Class Support and Prediction Proportions",
       x = "Truth",
...
write.csv(classifier_metrics_tab, '../concordance_data/iMUBAC_batch_3_pred_acc.csv',
          row.names = F)

# write plot data
write.csv(data, '../concordance_data/iMUBAC_batch_3_pred_data.csv',
          row.names = F)
```
...
write.csv(dat_final, '../concordance_data/iMUBAC_all_batches.csv', row.names = F)
```
