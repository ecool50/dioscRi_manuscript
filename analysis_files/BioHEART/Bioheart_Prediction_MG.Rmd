...
```{r}
load('../../../Analysis_data/sce_objects/BioHEART/study_4_MMD_VAE_updated.RData')
load('../../../Analysis_data/sce_objects/BioHEART/study_3_MMD_VAE_updated.RData')
```
...
```{r}
df_raw %>% filter(mg_cell_type_distinct == "CD4+ Treg") %>% 
    ggplot(aes(y = CD14, x = as.factor(gensini_bin))) + 
    geom_boxplot()
```

```{r, eval=FALSE}
colData <- as.data.frame(SummarizedExperiment::colData(sce_norm))
markerData <- colData %>% dplyr::select(sample_id = "sample_id", 
            "mg_cell_type_distinct") %>% cbind(data.frame(t(SummarizedExperiment::assay(sce_norm, 
            assay)), check.names = FALSE)) %>% dplyr::select(sample_id, 
            "mg_cell_type_distinct", useMarkers)
```
...
ggplotify::as.ggplot(scater::plotGroupedHeatmap(sce_norm_sub, 
  ...
  # theme_bw() +
  # theme_minimal() +
  ggtitle(label = "Training manually gated clusters expression") +
    theme(
...
# save(stats_mean, file = "../../../discovery_cohort_sig_stats_mg.rda")
