...
sce_subdir <- file.path(external_data_root, "sce_objects", "COVID_19_PBMC_Mathew_2020")
```
...
```{r}
metadata <- read.csv(file.path(sce_subdir, 'Covid_Bcell_metadata.csv'))
...
```{r}
load(file.path(sce_subdir, 'bcell_down_sampled_train_MMD_VAE_sce.RData'))
load(file.path(sce_subdir, 'bcell_down_sampled_test_MMD_VAE_sce.RData'))
```
